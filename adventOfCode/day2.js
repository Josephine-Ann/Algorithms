function passwords(inp) {
    let splitItem;
    let numbers;
    let pass = {}
    let c = 0;
    let counter = 0;
    inp = inp.map((element, index) => {
        splitItem = element.split(" ")
        numbers = splitItem[0].split("-")
        splitItem[1] = splitItem[1][0]
        pass[index] = {
            min: parseInt(numbers[0], 10),
            max: parseInt(numbers[1], 10),
            letter: splitItem[1],
            word: splitItem[2].split("")
        }
    });
    let newRulesCounter = 0
    let firstIndice;
    let secondIndice;
    let countConditions = 0;
    for (const prop in pass) {
        c = (pass[prop]["word"].filter(l => l === pass[prop]["letter"])).length
        if (c >= pass[prop]["min"] && c <= pass[prop]["max"]) {
            counter++
        }
        firstIndice = pass[prop]["min"] - 1
        secondIndice = pass[prop]["max"] - 1
        if (pass[prop]["word"][firstIndice] === pass[prop]["letter"]) countConditions++
        if (pass[prop]["word"][secondIndice] === pass[prop]["letter"]) countConditions++
